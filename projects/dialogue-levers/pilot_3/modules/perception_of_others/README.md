# Perception-of-others module

Seven-item Likert battery capturing a participant's perceptions of a
specific other person (typically a discussion partner). Adapted from the
[Watts-Lab `perceptionOfOthers` survey](https://github.com/Watts-Lab/surveys/tree/main/surveys/perceptionOfOthers)
— the Watts-Lab README is preserved verbatim below.

**Note:** the upstream's `liking` item was pulled from this module on
2026-05-11 and consolidated into the new `overall_affect` module as
the realized-liking component of the affect composite. The `common`
item (perceived similarity) is intentionally kept here — it measures
a different construct from `overall_affect/connection` (felt-bond).
See ADR
...
├── perception_of_others.stagebook.yaml ← module entry point (one template)
├── self_perspective_taking.prompt.md
├── common.prompt.md
├── polite.prompt.md
├── listening.prompt.md
├── partner_perspective_taking.prompt.md
├── talking.prompt.md
├── defensive.prompt.md
└── README.md (this file)
```
...
|---|---|---|---|
| 1 | `self_perspective_taking` | How easy was it for *you* to see *their* point of view? | Positive |
| 2 | `common` | How much do you think you have in common with them? | Positive |
| 3 | `polite` | How polite were they during the discussion? | Positive |
| 4 | `listening` | How well did they listen to you? | Positive |
| 5 | `partner_perspective_taking` | How well were *they* able to see *your* point of view? | Positive |
| 6 | `talking` | How much did they talk during the discussion? | **U-shaped** (0.5 = appropriate) |
| 7 | `defensive` | Did they get defensive or resistant to differing views? | **Negative** |

All seven items are five-point Likert and required in the source
schema. Display order in the module matches the table above.
...
### 2. Add a "who to think about" instruction before invoking

The battery is written for a generic "them/they". Precede the invocation
with an instruction telling the participant which person to keep in
mind, e.g.:
...
If the battery is asked about multiple targets (e.g. each member of a
group), invoke once per target with distinct prefixes.

### 3. Storage keys

With `prefix: perception_of_partner`, the module produces:

- `prompt_perception_of_partner_self_perspective_taking`
- `prompt_perception_of_partner_common`
- `prompt_perception_of_partner_polite`
- `prompt_perception_of_partner_listening`
- `prompt_perception_of_partner_partner_perspective_taking`
- `prompt_perception_of_partner_talking`
- `prompt_perception_of_partner_defensive`

## Field-name and encoding differences from the Watts-Lab source

**Field names.** The upstream JSON uses inconsistent camelCase
prefixing (`liking`, `selfPerspectiveTaking`, `partnerListening`,
`partnerTalking`, `partnerPerspectiveTaking`, `defensive`, …). This
module uses consistent snake_case with explicit `self_*` / `partner_*`
prefixes only where the direction matters (the two perspective-taking
items), since the entire battery is otherwise about the partner — the
call-site `${prefix}` already namespaces the items.

| Upstream | This module |
|---|---|
| `liking` | `liking` |
| `selfPerspectiveTaking` | `self_perspective_taking` |
| `common` | `common` |
| `polite` | `polite` |
| `partnerListening` | `listening` |
| `partnerPerspectiveTaking` | `partner_perspective_taking` |
| `partnerTalking` | `talking` |
| `defensive` | `defensive` |

**Numeric encoding.** Each item is in stagebook's numeric multipleChoice
mode (`- 0: Very poorly` … `- 1: Very well`), so participants see the
text labels and stagebook stores both the label and a 0–1 normalized
value. The upstream uses 1–5 integer values (`"value": "1"` … `"value":
"5"` as strings).

**Numeric direction matches valence.** For all monotone items the
encoding goes **higher = more positive experience** — researchers can
average them into a receptive-behavior composite without per-item
inversion. One item (`defensive`) is reverse-coded relative to upstream
display order to keep this convention:

| Item | Direction | Upstream conversion |
|---|---|---|
| `defensive` | high = less defensive | `((5 - upstream) - 1) / 4` |
| all other monotone items | high = more positive | `(upstream - 1) / 4` |

The participant sees the same display order as the upstream schema for
all items; only the `defensive` item's stored numeric values are
flipped. `listening`, `partner_perspective_taking`, and `defensive` all
now read "high = receptive" and can be averaged directly.

### Special case: `talking`

**`talking` is U-shaped, not monotone.** The numeric encoding follows
display order: 0 = "Hardly at all", 0.5 = "About the right amount", 1 =
"Far too much". The raw value is not directly interpretable as
better-or-worse; for a "talked appropriately" composite, transform via
`1 - abs(0.5 - value) * 2` (1 at midpoint, 0 at either extreme).

> Note: this item is a candidate for replacement with two
> monotonically-valenced items (e.g. "spoke too little" + "dominated
> the conversation") in a future revision of the upstream survey.

---

# Watts-Lab `perceptionOfOthers` README (preserved below)

The upstream README documents the literature trail for each construct
(liking, perspective-taking, polite, listening, talking, defensive,
commonality) plus the standard design-rationale notes (no "no opinion"
option, socially-desirable answer at the bottom of each list, etc.).
Included verbatim below; the screenshot referenced at the top is not
included here (the rendered prompts in this folder are the equivalent).
