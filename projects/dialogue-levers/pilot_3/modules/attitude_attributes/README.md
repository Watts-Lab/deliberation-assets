# Attitude attributes module

Seven-item meta-attitudinal battery measuring distinct dimensions of
attitude strength. Adapted from the
[Watts-Lab `attitudeAttributes` survey](https://github.com/Watts-Lab/surveys/tree/main/surveys/attitudeAttributes)
...
```
attitude_attributes/
├── attitude_attributes.stagebook.yaml  ← module entry point (one template)
├── importance.prompt.md
├── perceived_knowledge.prompt.md
├── certainty.prompt.md
├── ambivalence.prompt.md
├── personal_relevance.prompt.md
├── moral_conviction.prompt.md
├── perceived_elaboration.prompt.md
└── README.md (this file)
```

Each prompt file's `notes:` frontmatter contains a condensed pointer to
the relevant section of the preserved README below — so a researcher
reading any one prompt can quickly find the literature trail without
opening the README, and a researcher reading the README can find the
specific operationalization choice in the prompt file.

## What's in the battery

| Item                    | Question                                                                                          | Scale (low → high)                           |
| ----------------------- | ------------------------------------------------------------------------------------------------- | -------------------------------------------- |
| `importance`            | How important is this topic to you personally?                                                    | Not at all → Extremely important             |
| `perceived_knowledge`   | How well informed would you say you are about this topic?                                         | Not informed at all → Fully informed         |
| `certainty`             | How certain are you of your opinion on this topic?                                                | Not at all → Absolutely certain              |
| `ambivalence`           | Would you say that you are strongly on one side of this topic, or that your feelings are mixed?   | No mixed feelings → Extremely mixed feelings |
| `personal_relevance`    | To what extent does this topic have a direct impact on your life?                                 | Does not affect → Extreme effect             |
| `moral_conviction`      | How strongly is your opinion on this topic connected to your moral beliefs about right and wrong? | Not connected → Completely connected         |
| `perceived_elaboration` | In general, how much would you say you've thought about this topic?                               | Not at all → Extensively                     |

All seven are five-point Likert. None are reverse-coded. Display order
in the module matches the table above.
...
### 2. Invoke `attitude_attributes_questions` with a `prefix:` field

The prefix becomes part of every element's storage key, so you can
invoke the battery more than once in the same treatment without
key collisions (e.g. pre- and post-discussion).
...
gameStages:
  - name: attitude_attributes_pre
    duration: 300
    elements:
      # ... your introduction / topic display / etc. elements ...
      - template: attitude_attributes_questions
        fields:
          prefix: attitude_attributes_pre
      # ... your submitButton, if any, here ...
```

The module emits `contentType: elements` — a flat list of seven prompt
elements that the consumer embeds in their own stage. It does **not**
include a submitButton, because the parent stage usually has other
elements (instructions, topic display, audio cues) around the battery
and the call site is the right place to decide timing and required-field
logic.

### 3. Required-field conditions (if you add a submit button)

If you add a submit button at the call site and want to require all
seven responses before advancement, the conditions look like:

```yaml
- type: submitButton
  buttonText: Continue
  conditions:
    - { reference: self.prompt.<prefix>_importance, comparator: exists }
    - {
        reference: self.prompt.<prefix>_perceived_knowledge,
        comparator: exists,
      }
    - { reference: self.prompt.<prefix>_certainty, comparator: exists }
    - { reference: self.prompt.<prefix>_ambivalence, comparator: exists }
    - { reference: self.prompt.<prefix>_personal_relevance, comparator: exists }
    - { reference: self.prompt.<prefix>_moral_conviction, comparator: exists }
    - {
        reference: self.prompt.<prefix>_perceived_elaboration,
        comparator: exists,
      }
```

Replace `<prefix>` with the literal prefix value you passed in the
`fields:` block (Stagebook's reference checker resolves placeholders
inside `${...}` at template-expansion time; references that span across
the call site need the literal substituted value).

### 4. Resulting storage keys

With `prefix: attitude_attributes_pre`, the module produces:

- `prompt_attitude_attributes_pre_importance`
- `prompt_attitude_attributes_pre_perceived_knowledge`
- `prompt_attitude_attributes_pre_certainty`
- `prompt_attitude_attributes_pre_ambivalence`
- `prompt_attitude_attributes_pre_personal_relevance`
- `prompt_attitude_attributes_pre_moral_conviction`
- `prompt_attitude_attributes_pre_perceived_elaboration`

## Differences from the Watts-Lab source

The original
[`attitudeAttributes.json`](https://github.com/Watts-Lab/surveys/blob/main/surveys/attitudeAttributes/attitudeAttributes.json)
is a SurveyJS schema; this module is the Stagebook equivalent. Question
wording, scale-point labels, ordering, and required/optional designations
are preserved verbatim from the JSON. All seven items are marked
required in the source; required-field enforcement happens at the
consumer's submitButton (see "Required-field conditions" above).

**Numeric encoding.** Each item is in stagebook's numeric multipleChoice
mode (`- 0: Not at all important` … `- 1: Extremely Important`), so
participants see the text labels and stagebook stores both the label
and a 0–1 normalized numeric value. The upstream
`attitudeAttributes.score.js` uses 0–4 integer values for the same
five-point Likerts; multiply this module's stored value by 4 to convert
to the upstream encoding, or divide upstream by 4 to convert to ours.
Direction is consistent across all seven items — higher numeric value
always corresponds to "more of the construct" (more important, more
certain, more morally connected, etc.). No reverse-coded items.
...
(Nothing at all / A little bit / A moderate amount / A substantial amount / Almost Everything)
...
Many studies include some form of "I haven't thought much about this issue" as an alternative response (similar to "don't know") to an attitude position question. "Prior Elaboration" seems to get at this construct, and is what we are really interested in getting at.
